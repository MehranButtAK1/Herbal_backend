...
app.use(
  helmet({
    crossOriginResourcePolicy: { policy: "cross-origin" } // allow showing images if served elsewhere
  })
);
...
app.use(
  cors({
    origin(origin, cb) {
      // allow REST tools / server-to-server (no origin) and allowlisted origins
...
// Body parsing
app.use(express.json({ limit: "2mb" })); // supports base64 images
...
/* =========================
   4) Auth helpers
========================= */
// Issue JWT (10 minutes)
function signToken(user) {
  return jwt.sign({ sub: user._id, role: user.role }, JWT_SECRET, { expiresIn: "10m" });
}

// Check either:
//  • x-admin-password (plaintext) against seeded admin's hash
//  • OR Bearer JWT of admin role
async function adminGuard(req, res, next) {
  try {
    // Option A: x-admin-password (compatible with your current frontend)
    ...
    // Option B: Bearer JWT (role=admin)
...
/** Auth: optional (for panel use or future) */
app.post("/api/auth/login", loginLimiter, async (req, res, next) => {
  try {
    ...
    res.json({ token, role: user.role, expiresIn: 600 }); // 600s = 10m
  } catch (err) {
    next(err);
  }
});
